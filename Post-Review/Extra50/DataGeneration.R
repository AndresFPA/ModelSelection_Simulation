...
DataGeneration <- function(model, nclus, ngroups, N_g,
  ...
  par_table <- lavaan::lavaanify(model) # Create parameter table
  lat_var   <- lavaan::lavNames(par_table, "lv")
  obs_var   <- lavaan::lavNames(par_table, "ov")
  ...
  if (nclus == 1){
    B1 <- rep(x = reg_coeff, ngroups)
    B2 <- rep(x = reg_coeff, ngroups)
    B3 <- rep(x = reg_coeff, ngroups)
    B4 <- rep(x = reg_coeff, ngroups)
  }
